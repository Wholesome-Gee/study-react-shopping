...
- **new Set**으로 Array에 중복된 요소 제거하기
  - `new Set(arr)`은 arr의 중복된 요소를 제거하고 Set자료형을 return한다.
  - `Array.from(Set자료형)`은 Set자료형을 Array자료형으로 변환해준다.
  ```js
  let arr = [0, 0, 0, 1, 2, 3, 4, 5, 5, 5, 5];
  arr = new Set(arr); // Set(6) {0,1,2,3,4,5}
  arr = Array.from(arr); // [0,1,2,3,4,5]
  ```
